...
def load_data(use_dev=False, use_test=True):
    ...
    if use_dev:
        # For dev evaluation, use only train data as candidates to avoid self-matches
        candidate_prompts = train_prompts.copy()
        query_prompts = pd.read_csv(DEV_PROMPTS_FILE)
        print(f"Dev evaluation mode: {len(candidate_prompts)} candidates, {len(query_prompts)} queries")
    else:
        ...
        print(f"Test mode: {len(candidate_prompts)} candidates, {len(query_prompts)} queries")
...
def retrieve_responses(candidate_prompts, query_prompts):
    ...
    # Check for self-matches (for diagnostic purposes)
...
def main():
    ...
    if args.use_dev:
        output_file = os.path.join(DUMP_DIR, "track1_tfidf_dev.csv")
    elif args.use_train:
        output_file = os.path.join(DUMP_DIR, "track1_tfidf_train.csv")
    else:
        output_file = os.path.join(DUMP_DIR, "track1_tfidf_test.csv")
